...
param(
    ...
    [string]$JobName = "paper-harness-migration"
)
...
if ($LASTEXITCODE -ne 0) {
    throw "Cloud Run migration Job '$JobName' is unavailable."
}

& $Gcloud run jobs execute $JobName --project=$ProjectId --region=$Region --tasks=1 --wait
if ($LASTEXITCODE -ne 0) {
    throw "Cloud Run migration Job '$JobName' failed with exit code $LASTEXITCODE."
}
